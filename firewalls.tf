...
resource "aws_security_group" "admin_access_private" {
  ...
  description = "Allow All Private Access"
  vpc_id      = aws_vpc.main.id

  tags = {
    Name = "Internal Instances SG"
  }
}

resource "aws_security_group_rule" "admin_access_private_allow_all" {
